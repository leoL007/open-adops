...
基准率和每日样本量只从同一媒体、同一主指标的数据推导。CSV 可以映射 `Conversion Event / 目标转化事件`，用于确认通用 `Conversions` 列对应的注册、购买等事件；如果没有匹配的媒体、明确事件身份或指标分母，输入保持为空。Google、Meta、TikTok 与 AppsFlyer 的常见媒体别名会先合并，带小时的时间戳会先归一到日历日期，再计算活跃天数。

成本、收入、计数和复合指标不会套用这条比例公式。`CPI + 事件率` 等复合指标必须先拆成一个主要指标和独立护栏指标；否则显示 `not_calculable`，由媒体原生实验或预先定义的业务阈值判断。
