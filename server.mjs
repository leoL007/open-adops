...
3. suggestions 是候选需求，不会自动写入正式表。每条只生成文案、修改要求、规格和数量；asset_reference 必须固定输出空字符串，由优化师后续手动粘贴真实素材链接、文件名或说明。
4. 不强制写测试假设、单变量或成功指标；这些属于实验账本，不属于素材需求表。
5. 不得编造花费、CTR、CPI、CPA、ROAS、提升比例、行业基准或素材效果结论。输入没有文案或规格时，对应字段输出空字符串；数量未知时输出 null。
...
8. 本任务不包含寻找或匹配参考素材。即使输入中存在历史参考，候选需求的 asset_reference 也必须留空，避免误把上下文当成本轮指定素材。
