...
- Windows 新增可双击的 `OpenAdOps.cmd`：检查 Node.js、Codex CLI 与个人登录状态，并自动启动本地完整版。
- Release 自动附加 macOS / Windows 通用本地包与 SHA-256 校验文件，使用者不再需要从 Source code 判断下载方式。
- 投放优化结论拆成逐条动作，由优化师分别确认、执行、验证或拒绝，不再用一次 AI 诊断状态代替真实工作进度。
- 素材与归因类动作可转入素材需求或上线检查；其他动作继续留在优化清单中人工执行。
- 每条动作支持记录验证指标与结论；前后周期的相对变化由代码计算，AI 不代替优化师判断是否成功。
- 优化复盘可复制为 6 列飞书表格，报告输出同步读取已复核动作与验证结论。

### Fixed

- 前后周期缺少指标时保持“不可计算”，不再把空值误当成 0。

### Removed

- 下线独立实验台：移除导航、总览阶段、报告章节和优化动作流转入口；旧项目与备份中的实验数据继续保留，不做破坏性删除。
...
- 主页面只保留需求表；给设计的统一要求默认折叠，AI 输入、检查结果和候选建议收进右侧抽屉。
- 素材需求主交付改为「复制飞书表格」：复制 5 列富文本表格，直接粘贴到飞书云文档，不再输出从标题开始的长篇 Markdown。
- 飞书剪贴板表格不再注入灰色表头、固定宽度和粗体等样式，仅保留单元格文字居中，由飞书使用原生表格格式。
- 「给设计的统一要求」不再作为飞书表格首行复制；它只保留为项目全局备注，素材编号直接从 1 开始。
- 页头以「AI 生成素材建议」为主操作，手动新增与飞书复制作为编辑和交付操作。
- AI 建议保留「素材参考」列，并明确显示“AI 暂不提供素材参考”；采纳后字段留空，由优化师手动填写真实链接、文件名或说明。
- 制作方式、必须保留、禁止内容等历史字段在展示和导出时合并进「修改要求」，旧项目内容不会丢失。
- AI 改为「检查与补充」辅助层，输出缺项 / 风险提示和可选需求行，不再强制生成需求名称、素材类型等重复字段。
- 素材需求页面与飞书表格保持同一套精简列，不复制 AI 内部判断和重复元数据。
- 「投放执行方案」收敛为「上线执行」：页面仅保留上线摘要、监测与归因、Day 0–7 行动和上线检查。
- 上线摘要只读取搭建策略与素材需求的已确认结果，不再重复展示媒体策略、Campaign 蓝图或素材生产简报。
- 上线状态改为可上线 / 有条件上线 / 暂不可上线，不再展示 0–100 百分制就绪度。
- 隐藏执行方案文档/网页导出与本地版本入口；历史数据结构继续兼容，不影响已有项目和实验台。
...
- 三档分析模式：本地演示、Grok 4.5 与 GPT 5.6；Grok 通过本机 CLI 运行，GPT 5.6 保留 Terra / Sol 任务路由。
- 搭建策略改为可选工作流：不需要时可跳过，需要时可记录 Campaign / Ad group / Ad 参数并导出 Excel。
- 「素材方向」重建为「素材需求」：支持整理已有素材、生成新需求或本轮跳过。
- AI 读取上游项目与策略信息，输出「必须遵守 / 建议采用 / 待人工确认」事项和候选素材需求；候选仅在优化师明确采纳后进入正式表。
- 正式素材需求支持 Excel 导出，包含参考、文案、修改要求、规格、数量和制作边界。

### Changed

- 「客户追问」改为优化师自行确定的投放前策略清单，不再假设客户负责回答或执行投放。
- 素材页移除生产看板式负责人、截止日期与状态 UI，回到优化师撰写制作需求的真实流程。
- 项目阶段保留测试期、放量期与稳定期；修复阶段迁移导致放量、稳定项目被回退的问题。
- GPT 5.6、Terra 与 Sol 的模型名称和推理档在所有生成入口保持一致。

### Fixed

- 拒绝 Grok 返回的 `...`、`待补充` 等占位内容，避免空结果被当成成功写入项目。
- Grok 与 Codex CLI 在 CI Doctor 中均为可选依赖，GitHub Actions 不因缺少本机 CLI 失败。
...
- CSV 数据行列数与表头不一致时直接拒绝导入，不再静默丢弃多余值或补齐缺列。
- 数值解析只接受合法货币、千分位、百分号和会计负数格式；`1,2`、`1$2`、`1 2` 等损坏内容不再被拼成有效数字。
- 日期只接受完整日期或合法时间戳，尾随字符不再被截断成有效日期。
- 同时映射媒体安装与 AF 安装时，即使 AF 安装为 0，CVR 与 D1 留存也不会借用媒体安装作为 AF 口径。
- 三媒体新项目的初始预算占比固定合计为 100%，不再出现 `33 + 33 + 33 = 99`。
- 备份导入会提前拒绝缺少关键数组或对象的 AI 结果，避免损坏记录进入页面后触发渲染错误。
- 历史数据只完成日期或数值其中一项检查时，分别显示「未检查」，不再误报为未发现异常。

### Changed

- 演示项目也保存完整的数值与日期质量汇总；质量状态与提示统一由可测试的纯函数生成。

## [0.5.11] - 2026-07-24

### Added

- 投放优化页会持续显示空白数值与无效日期提示，不再只依赖一次性 toast。
- 工作台报告与独立 HTML 报告新增数据质量口径；旧数据未记录质量检查时明确标记。

### Changed

- 每次优化诊断会快照当时的日期与数值质量汇总，展开历史记录即可追溯；快照按固定字段清洗，不保留原始单元格内容。
