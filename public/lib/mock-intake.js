...
function makeField(key, value, source, status = "confirmed", evidence = "") {
  return {
    ...
    evidence: value ? text(evidence || "来自当前项目与粘贴资料") : "资料中未找到，由优化师核对或采用保守假设"
  };
}
...
export function buildMockIntake(project = {}, intake = {}, intent = "strategy") {
  ...
  const fields = [
    ...
    marketSource ? makeField("markets", markets, marketSource, "confirmed", "客户资料中的市场信息") : inferredField("markets", markets, markets ? "沿用项目设置，由优化师确认语言与国家范围" : ""),
    ...
    audienceSource ? makeField("audience", audience || "客户资料已描述目标用户", audienceSource, "confirmed", "客户资料中的用户描述") : inferredField("audience", audience, audience ? "沿用项目策略假设，由优化师确认" : ""),
  ...
  const preflightItemMap = {
    budget: ["确定内部可执行的月预算、币种与各市场上限；未确认前只输出预算场景，不写固定金额。", "预算口径决定首轮媒体数量、学习期和测试规模"],
    kpi: ["确定首阶段主指标；暂无可靠基线时保持“仅观察”，先积累数据再设置阈值。", "避免把安装量或未经验证的成本数字当成正式业务目标"],
    conversion_event: ["核对媒体、MMP 与业务后台的优化事件映射；未验证前不启用深层事件出价。", "优化事件决定 Campaign 结构、出价策略与学习质量"],
    timeline: ["确定上线窗口、测试周期与复盘节点；暂无硬截止时按首周学习节奏推进。", "时间边界决定素材、合规和数据准备节奏"],
    audience: ["确定首轮核心用户假设；资料不足时优先宽泛定向，用素材与转化事件筛选有效人群。", "用户假设决定媒体角色、素材角度和验证方式"],
    creative_supply: ["核对可用素材数量、格式与更新频率；产能不足时缩减媒体与测试矩阵。", "媒体组合必须匹配真实素材产能"],
    attribution: ["冻结媒体、MMP 与业务后台的统计口径；媒体用于实时优化，MMP 或业务后台用于结果判断。", "统一口径可以避免上线后的归因争议"],
    compliance: ["完成国家、牌照、免责声明、禁投人群与素材审核检查；未完成时不启用相关市场。", industry === "金融" ? "金融与受监管业务必须先确认合规边界" : "上线前需要确认平台政策、品牌安全和素材限制"]
  };
  ...
  const clarificationQuestions = fields
    .filter((field) => field.status !== "confirmed" && preflightItemMap[field.key])
    .sort((a, b) => questionOrder.indexOf(a.key) - questionOrder.indexOf(b.key))
    .slice(0, 7)
    .map((field, index) => ({
      field_key: field.key,
      question: preflightItemMap[field.key][0],
      reason: preflightItemMap[field.key][1],
      priority: ["conversion_event", "attribution"].includes(field.key) || (industry === "金融" && field.key === "compliance") ? "required" : "recommended"
    }));

  const assumptions = fields
    .filter((field) => field.status === "inferred")
    .map((field) => `暂定假设：${INTAKE_BRIEF_FIELDS.find(([key]) => key === field.key)?.[1]}为“${field.value}”，由优化师在投放前确认。`);
  ...
  return {
    executive_summary: `【演示】已把客户碎片资料整理为 ${fields.length} 个 Brief 字段；当前有 ${fields.filter((field) => field.status === "confirmed").length} 项已确认、${fields.filter((field) => field.status === "inferred").length} 项待确认、${fields.filter((field) => field.status === "missing").length} 项缺失。${intent === "questions" ? "建议优化师先处理投放前策略清单，再冻结策略初稿。" : "可先生成带假设的策略初稿，并由优化师在投放前处理关键缺口。"}`,
    brief_fields: fields,
    ...
    strategy_draft: {
      ...
      risks: [
        ...
        "所有 AI 推断必须由优化师确认，或明确标记为可验证假设后才能进入执行方案。"
      ]
    }
  };
}
