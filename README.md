...
OpenAdOps 是一个本地优先的 AI 付费媒体工作台，把客户 Offer、零散策略以及 Google Ads、Meta Ads、TikTok Ads 和 AppsFlyer 数据转化为结构化 Brief、投前作战包、素材测试、优化动作与客户报告。
...
12 项测试覆盖需求接收、Launch Pack 固定案例、金融合规阻塞、小预算媒体收敛、Brief 完整性、CSV 解析、字段识别、媒体 CPI 与 AppsFlyer CPI、指标聚合和 Schema 校验。测试过程不会调用真实模型。
...
产品定位、参考产品、固定案例、决策记录与发布方式分别见 [PRODUCT.md](./PRODUCT.md)、[产品参考](./docs/BENCHMARKS.md)、[验收案例](./docs/USER_CASES.md)、[决策记录](./docs/DECISIONS.md) 和 [发布规范](./docs/RELEASING.md)。
