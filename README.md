...
- **素材需求可交接**：AI 提示制作风险并提出候选需求，优化师采纳后形成带参考、文案、修改要求、规格与数量的 Excel
...
4. **素材需求**：继承上游资料，由 AI 提示制作边界与候选需求，优化师确认后导出 Excel
5. **投放执行方案**：Campaign 蓝图、素材生产简报、监测口径、上线检查项、首 7 天动作
6. **实验账本**：本轮 / 下轮 / 候选队列，样本门槛、停止规则、证据与学习
7. **投放优化**：导入 CSV，代码算 KPI，再基于证据生成诊断；记录采纳、执行与验证结论
8. **报告输出**：管理层 / 客户可读的 HTML 与打印 / PDF，包含最近优化决策记录
...
在「素材需求」页，把上游资料、优化师补充与 AI 建议整理为可交付的制作需求：

- 自动继承产品、市场、媒体、策略与需求接收信息
- 粘贴已有素材说明、参考链接、客户备注或本轮想法
- AI 输出「必须遵守 / 建议采用 / 待人工确认」事项和候选素材需求
- 记录素材参考、文案、修改要求、制作方式、媒体、市场、语言、规格、数量、必须保留与禁止内容
- AI 候选需由优化师逐条采纳；执行方案不会反向覆盖正式素材需求
- 支持整理已有素材、生成新需求或本轮跳过；可导出 Excel
...
当前 **136** 项自动化测试覆盖：需求接收、素材需求校验与 Excel 导出、投放执行方案、实验账本、优化决策历史、工作区备份与迁移保护、AI 请求错误、运行时版本校验、启动错误、请求地址解析、日历日期校验、CSV 行宽与数值质量、数据质量状态、预算占比、不可计算指标、Mock KPI 边界、静态服务边界、CSV 映射模板、周期对比、模型路由与标识、可选绩效目标、金融合规阻塞、小预算收敛、样本计算、转化事件身份、媒体别名合并、缺失数据保护、AF/媒体安装映射、媒体 CPI 与 AF-CPI、聚合与 Schema 校验。测试**不会**调用真实模型。
